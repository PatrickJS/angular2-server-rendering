...
import {ProtoViewRef, ViewRef, internalView, internalProtoView} from 'angular2/src/core/compiler/view_ref';
...
export class IsoAppViewManager {
  ...
  _createPooledView(protoView:viewModule.AppProtoView, componentTreeId: string):viewModule.AppView {
    var view = this._viewPool.getView(protoView);
    var id = protoView.protoChangeDetector.definition.id;

    if (isBlank(view)) {
      view = this._utils.createView(protoView, this._renderer.createView(protoView.render, componentTreeId), this, this._renderer);
  ...
  _createViewRecurse(view:viewModule.AppView, componentTreeId: string) {
    componentTreeId = componentTreeId || 'ng';

    var binders = view.proto.elementBinders;
    for (var binderIdx = 0; binderIdx < binders.length; binderIdx++) {
      var binder = binders[binderIdx];
      var subComponentTreeId = componentTreeId + '-' + (binderIdx + 1);
      if (binder.hasStaticComponent()) {
        var childView = this._createPooledView(binder.nestedProtoView, subComponentTreeId);
